...
#define OW_SYNCHRONIZE_NATURE       GEN_7 // In Gen8+, if a Pokémon with Synchronize leads the party, wild Pokémon will always have their same Nature as opposed to the 50% chance in previous games. Gift Pokémon excluded.
...
#define OW_ALTERED_TIME_RATIO           GEN_9       // In GEN_8_PLA, the time in game moves forward 60 seconds for every second in the RTC. In GEN_9, it is 20 seconds. TIME_DEBUG is 1:1, and meant for debugging purposes. This has no effect if OW_USE_FAKE_RTC is FALSE.
...
// Trainer Rematches
#define OW_REMATCH_BADGE_COUNT      4 // Number of badges necessary before the match call or vs seeker features allow rematches
