...
// Item config
#define I_SHINY_CHARM_ADDITIONAL_ROLLS  3           // The amount of additional Shiny rolls if the player has the Shiny Charm. Set it to 0 to disable the Shiny Charm's effects.
...
#define I_GRISEOUS_ORB_FORM_CHANGE      GEN_8  // In Gen9+, the Griseous Orb no longer changes Giratina's form when held.
...
#define I_POWER_ITEM_BOOST              GEN_6  // In Gen7+, Pokémon holding Power items gain 8 extra EVs instead of 4.
...
#define I_REPEL_INCLUDE_FAINTED         GEN_4  // In Gen1 and Gen6+, Repels use the level of the first party member to check which wild Pokémon to prevent encounters with, even if that member is fainted. In Gens2-5, they use the level of the first non-fainted Pokémon.
...
#define I_EXP_SHARE_FLAG        FLAG_ACTIVATE_EXP_SHARE           // If this flag is set, every Pokémon in the party will gain experience, regardless if they participated in the battle or not.
...
#define VAR_LAST_REPEL_LURE_USED    VAR_LAST_SPRAY_ITEM       // If this var has been assigned, the last Repel/Lure used will be saved and the player will be prompted with the vanilla repel YES/NO option, unless I_REPEL_LURE_MENU is set to TRUE.
...
// Vs. Seeker
#define I_VS_SEEKER_CHARGING        FLAG_ACTIVATE_SEEKER                     // If this flag is assigned, the Vs. Seeker functionality will be enabled. When the player has the Vs. Seeker, Match Call rematch functions will stop working. Documentation for the Vs. Seeker can be found in docs/tutorials/vs_seeker.md.
