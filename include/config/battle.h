...
#define B_CRIT_CHANCE               GEN_6 // Chances of landing a critical hit. See CalcCritChanceStage. Gen6+ chances guarantee a crit at 3 or more stages. Gen1 chances are based on half of the user's base Speed.
...
#define B_PARALYSIS_CHANCE          GEN_7 // In Champions, paralysis has a 12.5% chance of not moving, instead of 25%.
...
// Damage settings
#define B_BURN_DAMAGE               GEN_6 // In Gen1 and Gen7+, burn damage is 1/16 of max HP instead of 1/8. This also applies to Frostbite.
...
#define B_ROUGH_SKIN_DMG            GEN_3 // In Gen4+, Rough Skin's contact damage is 1/8 of max HP instead of 1/16. This also applies to Iron Barbs.
#define B_KNOCK_OFF_DMG             GEN_5 // In Gen6+, Knock Off deals 50% more damage if the target is holding an item that can be knocked off.
...
#define B_PRANKSTER_DARK_TYPES      GEN_5 // In Gen7+, Prankster-elevated status moves do not affect Dark-types.
#define B_SHEER_COLD_IMMUNITY       GEN_5 // In Gen7+, Ice-types are immune to Sheer Cold.
...
#define B_RECOIL_IF_MISS_DMG        GEN_2 // Crash damage's formula. See EFFECT_RECOIL_IF_MISS.
...
#define B_TOXIC_NEVER_MISS          GEN_5 // In Gen6+, Toxic bypasses accuracy checks when used by Poison-types.
...
#define B_CAN_SPITE_FAIL                GEN_3 // In Gen4+, Spite no longer fails if the target's last move has only 1 PP remaining.
...
#define B_PARTING_SHOT_SWITCH           GEN_6 // In Gen7+, the user won't switch out if Parting Shot fails to lower the target's stats.
...
#define B_BRICK_BREAK                   GEN_4 // In Gen4+, you can destroy your own side's screens. Additionally, in Gen5+, screens are no longer destroyed if the target is immune due to their typing.
#define B_WISH_HP_SOURCE                GEN_4 // In Gen5+, Wish heals by half of the user's max HP instead of the recipient's.
...
#define B_HEAL_BELL_SOUNDPROOF          GEN_5 // In Gen6+, Heal Bell affects inactive party members with Soundproof. In Gen5 only, it affects all party members, including active ones. In Gen4 and earlier, it does not affect any Pokémon with Soundproof.
...
#define B_PURSUIT_TARGET                GEN_3 // In Gen4+, Pursuit automatically targets and attacks any switching opponent. Previously, it only attacked a switching opponent if they were chose as the target.
...
#define B_SALT_CURE_DAMAGE              GEN_8 // In Gen9, Salt Cure deals 1/8 max HP (1/4 max HP to Steel and Water)
...
#define B_STEAL_WILD_ITEMS          GEN_7 // In Gen9+, Thief and Covet steal a wild Pokémon's item and send it to the Bag. In Gen8 and earlier, the stolen item would be held by the Thief/Covet user.
#define B_RESTORE_HELD_BATTLE_ITEMS GEN_LATEST // In Gen9+, all non-Berry items are restored after battle.
#define B_SOUL_DEW_BOOST            GEN_4 // In Gen7+, Soul Dew increases the power of the Latis' Psychic- and Dragon-type moves by ~20%. In Gen6 and earlier, it increased their Sp. Atk and Sp. Def by 50% each.
...
#define B_FLAG_NO_WHITEOUT          FLAG_NO_WHITEOUT     // If this flag is set, the player can not white out against Trainers. Please note that the party is not healed automatically!
...
#define B_THUNDERSTORM_TERRAIN      FALSE       // If set to TRUE, overworld Thunderstorm summons rain and Electric Terrain as in Gen8.
...
#define B_WAIT_TIME_MULTIPLIER              8         // This determines how long text pauses for in battle. Vanilla is 16. Lower values result in faster battles.
...
#define B_LOW_LEVEL_CATCH_BONUS         GEN_7 // In Gen9+, a bonus is added to the catch rate when catching a Pokémon lower than level 13. In Gen8, the bonus is applied to Pokémon lower than level 20.
#define B_MISSING_BADGE_CATCH_MALUS     GEN_7 // In Gen9+, a penalty is added to the catch rate when trying to catch a Pokémon 5 levels above the current obedience level, based on the number of Badges obtained.
...
#define B_CRITICAL_CAPTURE_IF_OWNED     GEN_7 // In Gen9+, a capture will appear critical if the Pokémon you're trying to catch is of a species that you have already caught.
...
#define B_SHOW_TYPES        SHOW_TYPES_CAUGHT     // When to show type indicators next to Pokémon HP bars, while choosing a move after selecting a target Pokémon.
...
#define B_POOL_RULE_MEGA_STONE_CLAUSE       TRUE    // Pick only 1 Pokémon with a Mega Stone.
