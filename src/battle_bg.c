...
static u8 GetBattleEnvironmentOverride(void)
{
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
        ...
        if (trainerClass == TRAINER_CLASS_LEADER || trainerClass == TRAINER_CLASS_MASTER)
...
void DrawBattleEntryBackground(void)
{
    ...
    else
    {
        if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
        {
            ...
            if (trainerClass == TRAINER_CLASS_LEADER || trainerClass == TRAINER_CLASS_CHAMPION || trainerClass == TRAINER_CLASS_MASTER)
            {
                LoadBattleEnvironmentEntryGfx(GetBattleEnvironmentOverride());
                return;
            }
...
void DrawTerrainTypeBattleBackground(void)
{
    ...
    case STATUS_FIELD_BUGGY_TERRAIN:
        LoadMoveBg(BG_BUGGY_TERRAIN);
        break;
    case STATUS_FIELD_FAULTY_TERRAIN:
        LoadMoveBg(BG_FAULTY_TERRAIN);
        break;
    case STATUS_FIELD_SPOOKY_TERRAIN:
        LoadMoveBg(BG_SPOOKY_TERRAIN);
        break;
    default:
        DrawMainBattleBackground();
        break;
