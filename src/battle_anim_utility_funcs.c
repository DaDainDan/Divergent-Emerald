...
void AnimTask_BlendBattleAnimPalExclude(u8 taskId)
{
    ...
    case ANIM_TARGET:
        ...
        if (IsDoubleBattle() && GetMoveTarget(gAnimMoveIndex) == TARGET_BOTH && IsBattlerSpriteVisible(GetPartnerBattler(gBattleAnimTarget)))
            animBattlers[1] = GetPartnerBattler(gBattleAnimTarget);
        break;
