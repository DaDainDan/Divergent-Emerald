...
static int GetTypeEffectivenessPoints(enum Move move, enum Species targetSpecies, int mode)
{
    ...
    if ((defAbility == ABILITY_LEVITATE || defAbility == ABILITY_EELEVATE || IsSpeciesAirborne(targetSpecies)) && moveType == TYPE_GROUND)
    {
    ...
    if (trainerId == TRAINER_FRONTIER_BRAIN || trainerId == TRAINER_PLAYER)
    {
        for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
        {
            ...
            for (j = 0; j < NUM_NATURE_STATS; j++)
            {
                ...
                if (gNaturesInfo[nature].statUp1 == j + 1 && gNaturesInfo[nature].statUp2 == j + 1)
                {
                    allocatedArray[j + NUM_STATS + 1] += (allocatedArray[j + 1] * 110) / 100;
                }
                else if (gNaturesInfo[nature].statUp1 == j + 1 || gNaturesInfo[nature].statUp2 == j + 1)
                {
        ...
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
            ...
            for (k = 0; k < FRONTIER_PARTY_SIZE; k++)
            {
                ...
                if (typeMultiplier == UQ_4_12(0))
                    moveScores[moveIndex] += 0;
                else if (typeMultiplier >= UQ_4_12(1.6))
                    moveScores[moveIndex] += movePower * 2;
                else if (typeMultiplier <= UQ_4_12(0.625))
                    moveScores[moveIndex] += movePower / 2;
                else
