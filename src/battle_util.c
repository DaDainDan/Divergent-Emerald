...
bool32 HandleMoveTargetRedirection(void)
{
     ...
     && !IsBattlerAlly(gBattlerAttacker, gSideTimers[side].followmeTarget))
    {
        ...
        return TRUE;
           ...
           && ((ability != ABILITY_LIGHTNING_ROD && ability != ABILITY_ORIGIN_OF_SEA && moveType == TYPE_ELECTRIC)
            || (ability != ABILITY_STORM_DRAIN && moveType == TYPE_WATER)))
    {
        ...
        if (redirectorOrderNum != MAX_BATTLERS_COUNT)
        {
            ...
            if ((battlerAbility == ABILITY_LIGHTNING_ROD || battlerAbility == ABILITY_ORIGIN_OF_SEA) && gCurrentMove != MOVE_TEATIME)
...
const u8 *CheckSkyDropState(u32 battler, enum SkyDropState skyDropState)
{
    ...
    if (gBattleMons[otherSkyDropper].status2 & STATUS2_LOCK_CONFUSE && skyDropState != SKY_DROP_STATUS_YAWN)
    {
        ...
        {
            // Set confused status
            gBattleMons[otherSkyDropper].status2 |= STATUS2_CONFUSION_TURN(((Random()) % 2) + 3);
...
void PrepareStringBattle(enum StringID stringId, u32 battler)
{
    ...
    else if ((stringId == STRINGID_DEFENDERSSTATFELL || stringId == STRINGID_PKMNCUTSATTACKWITH || stringId == STRINGID_PKMNCUTSSPECIALATTACKWITH || stringId == STRINGID_PKMNCUTSSPEEDWITH)
              && ((targetAbility == ABILITY_DEFIANT && CompareStat(gBattlerTarget, statId, MAX_STAT_STAGE, CMP_LESS_THAN))
                 || (targetAbility == ABILITY_STEADFAST && CompareStat(gBattlerTarget, STAT_SPEED, MAX_STAT_STAGE, CMP_LESS_THAN))) // ABILITY_COMPETITIVE
              ...
              && !(gBattleScripting.stickyWebStatDrop == 1 && gSideTimers[targetSide].stickyWebBattlerSide == targetSide)) // Sticky Web must have been set by the foe
    {
        ...
        if (targetAbility == ABILITY_STEADFAST) // ABILITY_COMPETITIVE
            SET_STATCHANGER(STAT_SPEED, 2, FALSE);
        else
            SET_STATCHANGER(statId, 2, FALSE);
    }
    else if (B_UPDATED_INTIMIDATE >= GEN_8 && (stringId == STRINGID_PKMNCUTSATTACKWITH || stringId == STRINGID_PKMNCUTSSPECIALATTACKWITH) && targetAbility == ABILITY_RATTLED
...
static void CancellerAsleep(u32 *effect)
{
    if (gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
    {
        ...
        {
            u8 toSub;
            if (GetBattlerAbility(gBattlerAttacker) == ABILITY_INNER_FOCUS) // ABILITY_EARLY_BIRD
                toSub = 2;
            else
                toSub = 1;
            ...
            if (gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
            {
                ...
                {
                    if (IsBattlerAlive(gBattlerAttacker) && !IsBattlerAtMaxHp(gBattlerAttacker) 
...
static void CancellerTruant(u32 *effect)
{
    if (GetBattlerAbility(gBattlerAttacker) == ABILITY_TRUANT && gDisableStructs[gBattlerAttacker].truantCounter)
    {
        ...
        gBattlescriptCurrInstr = BattleScript_MoveUsedLoafingAroundMsg;  // BattleScript_TruantLoafingAround
        gBattleStruct->moveResultFlags[gBattlerTarget] |= MOVE_RESULT_MISSED;
        *effect = 1;
    }
}
...
static void CancellerConfused(u32 *effect)
{
    ...
    if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION)
    {
        ...
        {
             // confusion dmg
            if (RandomPercentage(RNG_CONFUSION, (GetGenConfig(GEN_CONFIG_CONFUSION_SELF_DMG_CHANCE) >= GEN_7 ? 75 : 50))) // 33 : 50
            {
...
static bool32 TryChangeSideStatus(u32 battler, u32 statusFlag, u16 *timer)
{
    if ((!(gSideStatuses[GetBattlerSide(battler)] & statusFlag) && (!gBattleStruct->isSkyBattle)))
    {
...
static void ChooseStatBoostAnimation(u32 battler)
{
    ...
    for (stat = 0; stat < (NUM_BATTLE_STATS - 1); stat++) // Start loop at 1 to avoid STAT_HP
    {
...
bool32 CanAbilityBlockMove(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, u32 move, enum AbilityEffectOptions option)
{
    ...
    case ABILITY_OBLIVIOUS:
        if (IsSoundMove(move) && IsBattleMoveStatus(move) && !(GetBattlerMoveTargetType(battlerAtk, move) & MOVE_TARGET_USER))
        {
            if (gBattleMons[battlerAtk].status2 & STATUS2_MULTIPLETURNS)
                gHitMarker |= HITMARKER_NO_PPDEDUCT;
            battleScriptBlocksMove = BattleScript_SoundproofProtected;
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 move, u32 moveType, enum AbilityEffectOptions option)
{
    ...
    case ABILITY_WIND_RIDER:
        if ((IsWindMove(move) || moveType == TYPE_WIND) && !(GetBattlerMoveTargetType(battlerAtk, move) & MOVE_TARGET_USER))
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPATK;
        }
    ...
    case MOVE_ABSORBED_BY_BOOST_FLASH_FIRE:
        ...
        if (IsBattlerAtMaxHp(battlerDef) || (B_HEAL_BLOCKING >= GEN_5 && gStatuses3[battlerDef] & STATUS3_HEAL_BLOCK))
        {
            if (CompareStat(battlerDef, statId, MAX_STAT_STAGE, CMP_LESS_THAN) && !(gStatuses3[battlerDef] & STATUS3_HEAL_BLOCK))
            {
                if (gProtectStructs[battlerAtk].notFirstStrike)
                    battleScript = BattleScript_MoveStatDrain;
                else
        ...
        else if (!(IsBattlerAtMaxHp(battlerDef)))
        {
            if (gProtectStructs[battlerAtk].notFirstStrike)
                battleScript = BattleScript_MoveHPDrain;
            else
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_DOWNLOAD:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                u32 statId = STAT_SPATK; 
                u32 diagonalBattler = BATTLE_OPPOSITE(battler);
                
                if (IsDoubleBattle() && IsBattlerAlive(BATTLE_PARTNER(diagonalBattler)))
                    diagonalBattler = BATTLE_PARTNER(diagonalBattler);

                if (IsBattlerAlive(diagonalBattler))
                {
                    switch (GetHighestStatId(diagonalBattler))
                    {
                    case STAT_ATK:
                        statId = STAT_DEF;
                        break;
        ...
        case ABILITY_MOODY:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                ...
                {
                    gBattleScripting.savedBattler = gBattlerAttacker; // Save the current attacker
                    gBattlerAttacker = battler;
                    do
                    {
        ...
        case ABILITY_SILK_SPEW:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                ...
                BattleScriptPushCursorAndCallback(BattleScript_SilkSpewActivates);
                effect++;
            }
            break;
        ...
        case ABILITY_TANGLED_FEET:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                ...
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_TangledFeetConfusion);
                effect++;
            }
            break;
        ...
        case ABILITY_WHITE_SMOKE:
            if (!gSpecialStatuses[battler].switchInAbilityDone
                && TryChangeSideStatus(battler, SIDE_STATUS_MIST, &gSideTimers[GetBattlerSide(battler)].mistTimer))
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_WhiteSmokeActivates);
                effect++;
            }
            break;
        case ABILITY_VICTORY_STAR:
            if (!gSpecialStatuses[battler].switchInAbilityDone
                && TryChangeSideStatus(battler, SIDE_STATUS_LUCKY_CHANT, &gSideTimers[GetBattlerSide(battler)].luckyChantTimer))
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_VictoryStarActivates);
                effect++;
            }
            break;
        ...
        case ABILITY_PROVOCATEUR:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                ...
                if (GetBattlerAbility(opposingBattler) != ABILITY_OBLIVIOUS && GetBattlerAbility(opposingBattler) != ABILITY_INNER_FOCUS
                    && !IsAbilityOnSide(opposingBattler, ABILITY_AROMA_VEIL)
                    && gDisableStructs[opposingBattler].tauntTimer == 0)
                {
                    gBattlerAttacker = battler;
                    gBattlerTarget = opposingBattler;
                    gDisableStructs[opposingBattler].tauntTimer = 3;
                    gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                    BattleScriptPushCursorAndCallback(BattleScript_ProvocateurActivates);
                    effect++;
                }
            }
            break;
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            // Dry Skin works similarly to Rain Dish in Rainc
            case ABILITY_HYDRATION:
            case ABILITY_RAIN_DISH:
                 ...
                 && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                {
                    ...
                    BattleScriptPushCursorAndCallback(BattleScript_IceBodyHeal);
                    gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / (gLastUsedAbility == ABILITY_HYDRATION ? 16 : 8); // (gLastUsedAbility == ABILITY_RAIN_DISH ? 16 : 8)
                    if (gBattleStruct->moveDamage[battler] == 0)
                        gBattleStruct->moveDamage[battler] = 1;
                    gBattleStruct->moveDamage[battler] *= -1;
                    effect++;
            ...
            case ABILITY_CHLOROPHYLL:
                if (IsBattlerWeatherAffected(battler, B_WEATHER_SUN)
                 ...
                 && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                {
                    gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SUNHEAL;
                    BattleScriptPushCursorAndCallback(BattleScript_IceBodyHeal);
                    gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / 16;
                    if (gBattleStruct->moveDamage[battler] == 0)
                        gBattleStruct->moveDamage[battler] = 1;
            ...
            case ABILITY_SHED_SKIN:
                if ((gBattleMons[battler].hp >= gBattleMons[battler].maxHP / 4)
                    && (gBattleMons[battler].status1 & STATUS1_ANY) && !(gBattleMons[battler].status1 & STATUS1_SLEEP))
                //  && (B_ABILITY_TRIGGER_CHANCE > GEN_3 ? RandomPercentage(RNG_SHED_SKIN, 30) : RandomChance(RNG_SHED_SKIN, 1, 3)))
                {
            ...
            case ABILITY_SPEED_BOOST:
                if (CompareStat(battler, STAT_SPEED, MAX_STAT_STAGE, CMP_LESS_THAN) && GetBattlerTurnOrderNum(battler) != 0 // gDisableStructs[battler].isFirstTurn != 2
            ...
            case ABILITY_COMATOSE:
                ...
                if (!IsBattlerAtMaxHp(battler)
                 && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                { 
                    BattleScriptPushCursorAndCallback(BattleScript_ComatoseHealing);
                    ...
                    if (gBattleStruct->moveDamage[battler] == 0)
                        gBattleStruct->moveDamage[battler] = 1;
                    if (!(gStatuses3[battler] & STATUS2_NIGHTMARE))
                        gBattleStruct->moveDamage[battler] *= -1;
                    effect++;
                }
                break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_MUMMY:
            if (!(gBattleStruct->moveResultFlags[battler] & MOVE_RESULT_NO_EFFECT)
             ...
             && !IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_GHOST, TYPE_UNDEAD)
        ...
        case ABILITY_EFFECT_SPORE:
        {
             ...
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_SAFETY_GOGGLES)
            {
                ...
                if (ability == ABILITY_EFFECT_SPORE)
                {
                    poison = 15; // = 10
                    paralysis = 25; // = 20
                    sleep = 30;
        ...
        case ABILITY_CUTE_CHARM:
            ...
            //  && (B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? RandomPercentage(RNG_CUTE_CHARM, 30) : RandomChance(RNG_CUTE_CHARM, 1, 3))
             ...
             && GetBattlerAbility(gBattlerAttacker) != ABILITY_OBLIVIOUS && GetBattlerAbility(gBattlerAttacker) != ABILITY_INNER_FOCUS
            //  && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
            //  && IsMoveMakingContact(move, gBattlerAttacker)
             && !IsAbilityOnSide(gBattlerAttacker, ABILITY_AROMA_VEIL))
            {
        ...
        //     {
        //         gEffectBattler = battler;
        //         SET_STATCHANGER(STAT_SPEED, 3, FALSE);
        //         BattleScriptPushCursor();
        //         gBattlescriptCurrInstr = BattleScript_TargetAbilityStatRaiseRet;
        //         effect++;
        //     }
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_EFFECT_SPORE:
        {
            ...
            if ((!IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_GRASS) || B_POWDER_GRASS < GEN_6)
             && ability != ABILITY_OVERCOAT
             && GetBattlerHoldEffect(gBattlerTarget, TRUE) != HOLD_EFFECT_SAFETY_GOGGLES)
            {
                ...
                i = RandomUniform(RNG_EFFECT_SPORE_ATTACK, 0, B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? 99 : 299);
                if (i < poison)
                    goto POISON_TOUCH;
                if (i < paralysis)
                    goto STATIC_ATTACK;
        ...
        case ABILITY_POWER_OF_ALCHEMY:
            ...
            && IsBattlerAlive(gBattlerTarget)
            && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
            && GetMoveType(gCurrentMove) == TYPE_POISON
            && IsBattlerTurnDamaged(gBattlerTarget) // Need to actually hit the target
            && (RandomPercentage(RNG_POWER_OF_ALCHEMY, 50))) 
            {
                ...
                static const u32 sPowerOfAlchemyEffects2[] =
                    {
                ...
                if (!(gBattleMons[gBattlerTarget].status1))
                    gBattleScripting.moveEffect = RandomElement(RNG_POWER_OF_ALCHEMY_EFFECT, sPowerOfAlchemyEffects);
                else
                    gBattleScripting.moveEffect = RandomElement(RNG_POWER_OF_ALCHEMY_EFFECT, sPowerOfAlchemyEffects2);
                    
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_AbilityStatusEffectNoPopUp;
                gHitMarker |= HITMARKER_STATUS_ABILITY_EFFECT;
                effect++;
            }
        case ABILITY_DOOM_DESIRE:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && IsBattlerTurnDamaged(gBattlerTarget)
             && IsBattlerAlive(gBattlerTarget)
             && GetMoveType(gCurrentMove) == TYPE_GHOST
        ...
        case ABILITY_IRON_BARBS:
             ...
             && IsMoveMakingContact(move, gBattlerAttacker))
            {
              gBattleStruct->moveDamage[gBattlerAttacker] = GetNonDynamaxMaxHP(gBattlerAttacker) / (B_ROUGH_SKIN_DMG >= GEN_4 ? 8 : 16);
                ...
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_RoughSkinActivatesAttacker;
                effect++;  
            }
            break;
        case ABILITY_MUMMY:
            if (!(gBattleStruct->moveResultFlags[battler] & MOVE_RESULT_NO_EFFECT)
             ...
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
             && IsMoveMakingContact(move, gBattlerAttacker)
             && gDisableStructs[gBattlerTarget].overwrittenAbility != GetBattlerAbility(gBattlerAttacker)
             && gBattleMons[gBattlerTarget].ability != ABILITY_MUMMY
             && gBattleMons[gBattlerTarget].ability != ABILITY_LINGERING_AROMA
             && !IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_GHOST, TYPE_UNDEAD)
        ...
        case ABILITY_FLAME_BODY:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && (B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? RandomPercentage(RNG_FLAME_BODY_ATTACK, 30) : RandomChance(RNG_FLAME_BODY_ATTACK, 1, 3)))
            {
                gBattleScripting.moveEffect = MOVE_EFFECT_AFFECTS_USER | MOVE_EFFECT_BURN;
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptPushCursor();
        ...
        case ABILITY_ICE_BODY:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && (IsMoveMakingContact(move, gBattlerAttacker))
             && IsBattlerTurnDamaged(gBattlerTarget)
             && CanGetFrostbite(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerTarget))
        ...
        case ABILITY_TANGLING_HAIR:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             && IsBattlerAlive(gBattlerTarget)
             && (CompareStat(gBattlerTarget, STAT_SPEED, MIN_STAT_STAGE, CMP_GREATER_THAN) || GetBattlerAbility(gBattlerTarget) == ABILITY_MIRROR_ARMOR)
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && IsBattlerTurnDamaged(gBattlerTarget)
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
             && IsMoveMakingContact(move, gBattlerAttacker))
            {
                ...
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_GooeyActivatesAttack;
                gHitMarker |= HITMARKER_STATUS_ABILITY_EFFECT;
                effect++;
            }
            break;
        case ABILITY_GOOEY:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && IsBattlerTurnDamaged(gBattlerTarget)
             && ((GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS && IsMoveMakingContact(move, gBattlerAttacker))
             || moveType == TYPE_WATER)
             )
            {
                SET_STATCHANGER(STAT_SPEED, 1, TRUE);
        ...
        case ABILITY_MOON_PRESENCE:
                ...
                && (CompareStat(gBattlerAttacker, STAT_SPATK, MAX_STAT_STAGE, CMP_LESS_THAN)))
            {
                ...
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_MoonPresence;
            }
            break;
        }
        break;
    ...
    case ABILITYEFFECT_SYNCHRONIZE:
        if (gLastUsedAbility == ABILITY_RECIPROCAL && (gHitMarker & HITMARKER_SYNCHRONIZE_EFFECT))
        {
            ...
            RecordAbilityBattle(gBattlerTarget, ABILITY_RECIPROCAL);
    ...
    case ABILITYEFFECT_ATK_SYNCHRONIZE:
        if (gLastUsedAbility == ABILITY_RECIPROCAL && (gHitMarker & HITMARKER_SYNCHRONIZE_EFFECT))
        {
            ...
            RecordAbilityBattle(gBattlerAttacker, ABILITY_RECIPROCAL);
...
bool32 IsMoldBreakerTypeAbility(u32 battler, u32 ability)
{
    ...
    return (ability == ABILITY_MOLD_BREAKER || ability == ABILITY_TERAVOLT || ability == ABILITY_TURBOBLAZE
        || (ability == ABILITY_MYCELIUM_MIGHT && IsBattleMoveStatus(gCurrentMove)) || ability == ABILITY_COSMIC);
}
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, enum MoveEffects effect, enum NonVolatileStatus option)
{
    ...
    case MOVE_EFFECT_SLEEP:
        ...
        else if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_GHOST, TYPE_UNDEAD))
        {
            battleScript = BattleScript_NotAffected;
        }
    ...
    case MOVE_EFFECT_FROSTBITE:
        ...
        else if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_ICE, TYPE_FIRE) || IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN))
        {
            battleScript = BattleScript_NotAffected;
        }
    ...
    else if (IsBattlerTerrainAffected(battlerDef, STATUS_FIELD_MISTY_TERRAIN) && effect != MOVE_EFFECT_SLEEP)
    {
...
bool32 HasEnoughHpToEatBerry(u32 battler, u32 hpFraction, u32 itemId)
{
    ...
    if (GetBattlerAbility(battler) == ABILITY_GLUTTONY && isBerry
...
{
    if (HasEnoughHpToEatBerry(battler, (B_CONFUSE_BERRIES_HEAL >= GEN_7 ? 3 : 2), itemId)
...
{
    if (HasEnoughHpToEatBerry(battler, 3, itemId))
    {
...
u32 GetBattleMoveTarget(u16 move, u8 setTarget)
{
    ...
    case MOVE_TARGET_OPPONENT:
        ...
        else
        {
            ...
            if (moveType == TYPE_ELECTRIC && GetBattlerAbility(targetBattler) != ABILITY_LIGHTNING_ROD && GetBattlerAbility(targetBattler) != ABILITY_ORIGIN_OF_SEA)
            {
                if (IsAbilityOnField(ABILITY_ORIGIN_OF_SEA) > 0)
                {
                    if (B_REDIRECT_ABILITY_ALLIES >= GEN_4)
                        battlerAbilityOnField = IsAbilityOnField(ABILITY_ORIGIN_OF_SEA);
                    else
                        battlerAbilityOnField = IsAbilityOnOpposingSide(targetBattler, ABILITY_ORIGIN_OF_SEA);
                }
                else
                {
                    if (B_REDIRECT_ABILITY_ALLIES >= GEN_4)
...
u8 GetAttackerObedienceForAction()
{
    ...
    if (B_OBEDIENCE_MECHANICS < GEN_8 && !IsOtherTrainer(gBattleMons[gBattlerAttacker].otId, gBattleMons[gBattlerAttacker].otName))
        return OBEYS;
    if (gSpeciesInfo[gBattleMons[gBattlerAttacker].species].isLegendary && !FlagGet(FLAG_IS_CHAMPION))
...
{
    return (gSpeciesInfo[gBattleMons[battler].species].airborne);
}
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    if (gProtectStructs[battlerAtk].helpingHand)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (gBattleMons[battlerAtk].status2 & STATUS2_FLINCHED)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_MINUS:
        ...
        break;
    case ABILITY_FLOWER_GIFT:
        if (IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN) && IsBattleMoveSpecial(move)) // gBattleMons[battlerAtk].species == SPECIES_CHERRIM_SUNSHINE && 
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.2));
        break;
    // case ABILITY_HUSTLE:
    //     if (IsBattleMovePhysical(move))
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
            if (IsBattlerWeatherAffected(BATTLE_PARTNER(battlerAtk), B_WEATHER_SUN) && IsBattleMoveSpecial(move)) // gBattleMons[BATTLE_PARTNER(battlerAtk)].species == SPECIES_CHERRIM_SUNSHINE && 
...
static inline u32 CalcDefenseStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectDef, u32 weather)
{
    ...
    case ABILITY_SNOW_CLOAK:
        if (IsBattlerWeatherAffected(battlerDef, B_WEATHER_HAIL | B_WEATHER_SNOW) && usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_SAND_VEIL:
        if (IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM) && !usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_FLOWER_GIFT:
        if (IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN) && !usesDefStat) // gBattleMons[battlerDef].species == SPECIES_CHERRIM_SUNSHINE && 
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.25));
        break;
    case ABILITY_LEAF_GUARD:
        if (IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.20));
        break;
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerDef)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
            if (IsBattlerWeatherAffected(BATTLE_PARTNER(battlerDef), B_WEATHER_SUN) && !usesDefStat) // gBattleMons[BATTLE_PARTNER(battlerDef)].species == SPECIES_CHERRIM_SUNSHINE && 
                modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.15));
            break;
        }
    }
...
static uq4_12_t GetWeatherDamageModifier(struct DamageCalculationData *damageCalcData, enum ItemHoldEffect holdEffectAtk, enum ItemHoldEffect holdEffectDef, u32 weather)
{
    ...
    {
        if (moveType != TYPE_FIRE && moveType != TYPE_WATER && moveType != TYPE_ELECTRIC)
            return UQ_4_12(1.0);
        return (moveType == TYPE_FIRE) ? UQ_4_12(0.5) : UQ_4_12(1.3);
    }
    if (weather & B_WEATHER_SUN)
    {
        if (moveType != TYPE_FIRE && moveType != TYPE_GRASS && moveType != TYPE_WATER && moveType != TYPE_ICE)
            return UQ_4_12(1.0);
        return (moveType == TYPE_WATER || moveType == TYPE_ICE) ? UQ_4_12(0.5) : UQ_4_12(1.3);
    }
    if (weather & B_WEATHER_SNOW)
    {
        if ((moveType != TYPE_ICE && moveType != TYPE_GRASS) || (moveType == TYPE_GRASS && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_ICE, TYPE_FROST)))
            return UQ_4_12(1.0);
        return (moveType == TYPE_GRASS) ? UQ_4_12(0.5) : UQ_4_12(1.3);
    }
    if (weather & B_WEATHER_STRONG_WINDS)
    {
        if (moveType == TYPE_FLYING || gMovesInfo[move].windMove || moveType == TYPE_WIND)
            return UQ_4_12(1.3);
    }
    if (weather & B_WEATHER_SANDSTORM)
    {
        if (moveType == TYPE_SAND)
            return (abilityAtk == ABILITY_SAND_FORCE) ? UQ_4_12(1.6) : UQ_4_12(1.3);
    }
    return UQ_4_12(1.0);
}
...
static inline uq4_12_t GetBurnOrFrostBiteModifier(struct DamageCalculationData *damageCalcData, u32 abilityAtk)
{
    ...
    if (gBattleMons[battlerAtk].status1 & STATUS1_BURN
        ...
        && abilityAtk != ABILITY_GUTS && abilityAtk != ABILITY_INNER_FOCUS)
        return UQ_4_12(0.5);
...
static inline uq4_12_t GetZMaxMoveAgainstProtectionModifier(struct DamageCalculationData *damageCalcData)
{
    ...
    else if (GetBattlerAbility(damageCalcData->battlerAtk) == ABILITY_INFILTRATOR)
    {
        ...
        || (protected == PROTECT_QUICK_GUARD && !(GetChosenMovePriority(damageCalcData->battlerAtk, GetBattlerAbility(damageCalcData->battlerAtk)) > 0))
        || protected == PROTECT_CRAFTY_SHIELD
        || protected == PROTECT_MAX_GUARD)
            return UQ_4_12(1.0);
        else
...
static inline uq4_12_t GetScreensModifier(u32 move, u32 battlerAtk, u32 battlerDef, bool32 isCrit, u32 abilityAtk)
{
    ...
    {
        if (IsAbilityOnSide(battlerDef, ABILITY_FILTER))
...
static inline uq4_12_t GetDefenderAbilitiesModifier(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, uq4_12_t typeEffectivenessModifier, u32 abilityDef)
{
    ...
    case ABILITY_PRISM_ARMOR:
        ...
        break;
    case ABILITY_FLUFFY:
        if (!(IsBattleMovePhysical(move)) && moveType == TYPE_FIRE) // IsMoveMakingContact(move, battlerAtk)
            return UQ_4_12(1.6);
        if ((IsBattleMovePhysical(move) || moveType == TYPE_ICE) && moveType != TYPE_FIRE)
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, bool32 recordAbilities, uq4_12_t modifier, u32 defAbility)
{
    ...
    if (moveType == TYPE_FIRE && gDisableStructs[battlerDef].tarShot)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.6));
    if (moveType == TYPE_ELECTRIC && gSpeciesInfo[gBattleMons[battlerDef].species].airborne)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.6));
    ...
    else if ((moveType == TYPE_FIRE || moveType == TYPE_ICE || moveType == TYPE_GRASS) && defAbility == ABILITY_MAGMA_ARMOR)
    {
        modifier = UQ_4_12(0.0);
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(u16 move, u16 speciesDef, u16 abilityDef)
{
    uq4_12_t modifier = UQ_4_12(1.0);
    u32 moveType = GetBattleMoveType(move);
    bool8 IsAirborne = gSpeciesInfo[speciesDef].airborne;
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        if (moveType == TYPE_ELECTRIC && (abilityDef == ABILITY_LEVITATE || IsAirborne) && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(1.6);
        if (moveType == TYPE_GROUND && (abilityDef == ABILITY_LEVITATE || IsAirborne) && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(0.0);
...
uq4_12_t GetOverworldTypeEffectiveness(struct Pokemon *mon, u8 moveType)
{
    ...
    bool32 IsAirborne = gSpeciesInfo[speciesDef].airborne;
    ...
    if (moveType != TYPE_MYSTERY)
    {
        ...
        if (moveType == TYPE_ELECTRIC && IsAirborne)    
        {
            modifier = UQ_4_12(1.6);
        }
...
bool32 TestIfSheerForceAffected(u32 battler, u16 move)
{
    return ((GetBattlerAbility(battler) == ABILITY_SHEER_FORCE && MoveIsAffectedBySheerForce(move))
            || (GetBattlerAbility(battler) == ABILITY_POWER_OF_ALCHEMY && MoveIsAffectedBySheerForce(move) && GetMoveType(move) == TYPE_POISON));
}
...
bool32 IsBattlerWeatherAffected(u32 battler, u32 weatherFlags)
{
    if (gBattleWeather & weatherFlags && HasWeatherEffect())
    {
        ...
        if (GetBattlerHoldEffect(battler, TRUE) == HOLD_EFFECT_UTILITY_UMBRELLA) // gBattleWeather & (B_WEATHER_SUN | B_WEATHER_RAIN) && 
...
bool8 CanMonParticipateInSkyBattle(struct Pokemon *mon)
{
    ...
    bool8 IsAirborne = gSpeciesInfo[species].airborne;
    ...
    {
        if ((hasLevitateAbility || isFlyingType || IsAirborne) && !IsMonBannedFromSkyBattles(species))
            return TRUE;
    }
    return FALSE;
}
...
{
    if (ability == ABILITY_SHIELD_DUST || ability == ABILITY_SERENE_GRACE || ability == ABILITY_SHELL_ARMOR || ability == ABILITY_LEAF_GUARD)
    {
