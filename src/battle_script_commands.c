...
static void Cmd_critmessage(void)
{
    ...
    if (gBattleControllerExecFlags == 0)
    {
        if (gSpecialStatuses[gBattlerTarget].criticalHit && !IsBattlerUnaffectedByMove(gBattlerTarget))
        {
            ...
            {
                if (IsDoubleSpreadMove())
                    PrepareStringBattleWithWait(STRINGID_SUPERCRITONDEF, gBattlerTarget);
                else
                    PrepareStringBattleWithWait(STRINGID_SUPERCRIT, gBattlerAttacker);
            }
            else
            {
                if (IsDoubleSpreadMove())
...
static void Cmd_trysetrest(void)
{
    ...
    if (IsElectricTerrainAffected(gBattlerTarget, gFieldTimers.terrain))
    {
...
static void Cmd_setyawn(void)
{
    ...
    else if (IsElectricTerrainAffected(gBattlerTarget, gFieldTimers.terrain))
    {
    ...
    // else if (IsMistyTerrainAffected(gBattlerTarget, gFieldTimers.terrain))
    // {
...
void BS_JumpIfTerrainAffected(void)
{
    ...
    if (IsBattlerTerrainAffected(battler, gFieldTimers.terrain, cmd->flags))
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
...
bool8 CanSetHazard(enum BattleSide targetSide, enum Hazards hazard)
{
    switch (hazard)
    {
    case HAZARDS_SPIKES:
...
void BS_SetHazard(void)
{
    ...
    else if (gFieldTimers.terrain != B_TERRAIN_FAULTY)
    {
        if (cmd->primary)
