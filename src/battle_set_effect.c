...
static void HandleSetEffectConfusion(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    {
        gBattleMons[se->effectBattler].volatiles.confusionTimer = RandomUniform(RNG_CONFUSION_TURNS, 2, B_CONFUSION_TURNS); // 2-4 turns
...
{
    if (cv->abilities[se->effectBattler] == ABILITY_INNER_FOCUS || cv->abilities[se->effectBattler] == ABILITY_GUTS)
    {
        if (se->primary || se->certain)
        {
            gLastUsedAbility = cv->abilities[se->effectBattler];
            gBattlerAbility = se->effectBattler;
            RecordAbilityBattle(se->effectBattler, cv->abilities[se->effectBattler]);
            BattleScriptPush(se->script);
    ...
    else if (GetBattlerHoldEffect(se->effectBattler) == HOLD_EFFECT_FOCUS_BAND)
    {
        if (se->primary || se->certain)
        {
            gLastUsedItem = gBattleMons[se->effectBattler].item;
            RecordItemEffectBattle(se->effectBattler, HOLD_EFFECT_FOCUS_BAND);
            BattleScriptPush(se->script);
...
static void HandleSetEffectPayday(struct BattleCalcValues *cv, struct SetEffect *se)
{
    if (IsOnPlayerSide(cv->battlerAtk))
    {
        ...
        gPaydayMoney += (gBattleMons[cv->battlerAtk].level * 10);
...
static void HandleSetEffectStatChange(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    bool32 isDynamicStat = se->additionalEffect->random || se->additionalEffect->highest || se->additionalEffect->lowest;
    enum Stat dynamicStat = NUM_BATTLE_STATS;

    if (isDynamicStat)
        ResolveDynamicStat(se->effectBattler, cv->abilities[se->effectBattler], se->additionalEffect, &dynamicStat);
    ...
    for (enum Stat i = STAT_ATK; i < NUM_BATTLE_STATS; i++)
    {
        ...
        if (isDynamicStat)
        {
            if (stat != dynamicStat)
                continue;
            stage = GetDynamicStatValue(se->additionalEffect);
        }
        else
        {
            stage = GetStatStage(stat, se->additionalEffect);
        }
...
{
    if (cv->abilities[se->effectBattler] == ABILITY_STICKY_HOLD || gBattleMons[se->effectBattler].item == ITEM_NONE)
        return;
...
{
    enum BattlerId battler = IsAromaVeilProtected(se->effectBattler, GetBattlerAbility(cv->battlerAtk));
    ...
    {
        gBattleMons[se->effectBattler].volatiles.healBlockTimer = 3;
        BattleScriptPush(se->script);
...
{
    if (!IS_BATTLER_OF_TYPE(se->effectBattler, TYPE_GRASS) && !AreInnateImmunitiesNullified(se->effectBattler, cv->abilities[cv->battlerAtk]) 
     && !gBattleMons[se->effectBattler].volatiles.leechSeed)
    {
...
{
    if (CanSacrificeItem(cv->battlerAtk, cv->abilities[cv->battlerAtk], cv->move) || gBattleStruct->flungItem == FLUNG_ITEM_REMOVED)
    {
        ...
        case HOLD_EFFECT_CURSED_ORB:
            flingEffect.moveEffect = MOVE_EFFECT_CURSE;
            SetMoveEffect(cv, &flingEffect);
            break;
...
static void HandleSetEffectEscapeBind(struct BattleCalcValues *cv, struct SetEffect *se)
{
    if (!gBattleMons[se->effectBattler].volatiles.wrapped && !gBattleMons[se->effectBattler].volatiles.leechSeed)
    {
        gBattlescriptCurrInstr = se->script;
    }
    else if (IsBattlerAlive(se->effectBattler))
    {
        BattleScriptPush(se->script);
...
static void HandleSetEffectPlague(struct BattleCalcValues *cv, struct SetEffect *se)
{
...
static void HandleSetEffectYawnFoe(struct BattleCalcValues *cv, struct SetEffect *se)
{
    if (gBattleMons[se->effectBattler].volatiles.yawn == 0
     && cv->abilities[se->effectBattler] != ABILITY_OBLIVIOUS && cv->abilities[se->effectBattler] != ABILITY_INNER_FOCUS
     && CanBeSlept(cv->battlerAtk, se->effectBattler, cv->abilities[cv->battlerAtk], cv->abilities[se->effectBattler], BLOCKED_BY_SLEEP_CLAUSE)) // && RandomPercentage(RNG_G_MAX_SNOOZE, 50)
    {
...
{
    if (CanSetHazard(GetBattlerSide(se->effectBattler), HAZARDS_STEELSURGE))
    {
...
{
    if (CanSetHazard(GetBattlerSide(se->effectBattler), HAZARDS_STEALTH_ROCK))
    {
...
{
    if (CanSetHazard(GetBattlerSide(se->effectBattler), HAZARDS_TOXIC_SPIKES))
    {
...
{
    if (CanSetHazard(GetBattlerSide(se->effectBattler), HAZARDS_SPIKES))
    {
...
static void (*const sSetEffectHandlers[])(struct BattleCalcValues *cv, struct SetEffect *se) =
{
    ...
    [MOVE_EFFECT_CURSE] = HandleSetEffectNonVolatile,
    [MOVE_EFFECT_FROSTBITE_OR_SPD_MINUS_1] = HandleSetEffectNone,
    ...
    [MOVE_EFFECT_ESCAPE_BIND] = HandleSetEffectEscapeBind,
    [MOVE_EFFECT_PLAGUE] = HandleSetEffectPlague,
    [MOVE_EFFECT_SPELL_CHAIN] = HandleSetEffectNone,
    ...
    [MOVE_EFFECT_STRONG_WINDS] = HandleSetEffectWeather,
    [MOVE_EFFECT_SNOW] = HandleSetEffectWeather,
    [MOVE_EFFECT_MONSOON] = HandleSetEffectNone,
    ...
    [MOVE_EFFECT_BUGGY_TERRAIN] = HandleSetEffectTerrain,
    [MOVE_EFFECT_FAULTY_TERRAIN] = HandleSetEffectTerrain,
    [MOVE_EFFECT_SPOOKY_TERRAIN] = HandleSetEffectTerrain,
...
void SetMoveEffect(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    if (se->moveEffect == MOVE_EFFECT_FROSTBITE_OR_SPD_MINUS_1)
    {
        if (IS_BATTLER_OF_TYPE(cv->battlerAtk, TYPE_ICE))
            se->moveEffect = MOVE_EFFECT_FREEZE_OR_FROSTBITE;
        else
            se->moveEffect = MOVE_EFFECT_STAT_MINUS;
    }

    if (se->moveEffect == MOVE_EFFECT_MONSOON)
    {
        if ((gBattleWeather == B_WEATHER_NONE) || (gBattleWeather & B_WEATHER_SUN))
            se->moveEffect = MOVE_EFFECT_RAIN;
        else if (gBattleWeather & B_WEATHER_RAIN)
            se->moveEffect = MOVE_EFFECT_SUN;
        else
            se->moveEffect = MOVE_EFFECT_NONE;
    }

    if (se->moveEffect == MOVE_EFFECT_SPELL_CHAIN)
    {
        if (gBattleMons[cv->battlerAtk].volatiles.furyCutterCounter == 1)
            se->moveEffect = MOVE_EFFECT_STAT_MINUS;
        else
            se->moveEffect = MOVE_EFFECT_NONE;
    }
