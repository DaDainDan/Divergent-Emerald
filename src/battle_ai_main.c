...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (((GetMoveType(move) == TYPE_FIRE && GetMovePower(move) != 0) || CanBurnHitThaw(move)) && effectiveness < UQ_4_12(1.6) && (gBattleMons[battlerDef].status1 & (STATUS1_FROSTBITE | STATUS1_FREEZE)))
    {
        u32 aiMove;
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
    ...
    else if (effectiveness < UQ_4_12(0.625))
    {
        switch (moveEffect)
        {
    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        ...
        case ABILITY_SHIELDS_DOWN:
            if (IsShieldsDownProtected(battlerAtk, aiData->abilities[battlerAtk]) 
                && (IsNonVolatileStatusMove(move) && nonVolatileStatus != MOVE_EFFECT_SLEEP))
                RETURN_SCORE_MINUS(10);
            break;
        ...
        if (isDoubleBattle)
        {
            ...
            case ABILITY_SWEET_VEIL:
                ...
                break;
            case ABILITY_FLOWER_VEIL:
                if ((IS_BATTLER_OF_TYPE(battlerDef, TYPE_GRASS)) && (IsNonVolatileStatusMove(move) && nonVolatileStatus != MOVE_EFFECT_SLEEP)) // || IsStatLoweringEffect(moveEffect)))
                    RETURN_SCORE_MINUS(10);
                break;
            case ABILITY_AROMA_VEIL:
                if (IsAromaVeilProtectedEffect(moveEffect))
        ...
        {
            if ((IsNonVolatileStatusMove(move) && nonVolatileStatus != MOVE_EFFECT_SLEEP) || IsConfusionMoveEffect(moveEffect))
                RETURN_SCORE_MINUS(20);
        }
...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_ENCORE:
            if (GetActiveGimmick(battlerDef) == GIMMICK_DYNAMAX)
                ADJUST_SCORE(-10);
            else if (aiData->abilities[battlerDef] == ABILITY_OBLIVIOUS || aiData->abilities[battlerDef] == ABILITY_INNER_FOCUS)
                ADJUST_SCORE(-10);
        ...
        case EFFECT_PERISH_SONG:
            ...
            {
                if (CountUsablePartyMons(battlerAtk) == 0
                  && aiData->abilities[battlerAtk] != ABILITY_SOUNDPROOF && aiData->abilities[battlerAtk] != ABILITY_OBLIVIOUS
                  && aiData->abilities[BATTLE_PARTNER(battlerAtk)] != ABILITY_SOUNDPROOF && aiData->abilities[BATTLE_PARTNER(battlerAtk)] != ABILITY_OBLIVIOUS
                  && CountUsablePartyMons(FOE(battlerAtk)) >= 1)
                {
                    ADJUST_SCORE(-10); //Don't wipe your team if you're going to lose
                }
                else if ((!IsBattlerAlive(FOE(battlerAtk)) || (aiData->abilities[FOE(battlerAtk)] == ABILITY_SOUNDPROOF || aiData->abilities[FOE(battlerAtk)] == ABILITY_OBLIVIOUS)
                  || gStatuses3[FOE(battlerAtk)] & STATUS3_PERISH_SONG)
                  && (!IsBattlerAlive(BATTLE_PARTNER(FOE(battlerAtk))) 
                  || (aiData->abilities[BATTLE_PARTNER(FOE(battlerAtk))] == ABILITY_SOUNDPROOF || aiData->abilities[BATTLE_PARTNER(FOE(battlerAtk))] == ABILITY_OBLIVIOUS)
            ...
            {
                if (CountUsablePartyMons(battlerAtk) == 0 && aiData->abilities[battlerAtk] != ABILITY_SOUNDPROOF && aiData->abilities[battlerAtk] != ABILITY_OBLIVIOUS
                  && CountUsablePartyMons(battlerDef) >= 1)
                    ADJUST_SCORE(-10);

                if (gStatuses3[FOE(battlerAtk)] & STATUS3_PERISH_SONG 
                 || (aiData->abilities[FOE(battlerAtk)] == ABILITY_SOUNDPROOF || aiData->abilities[FOE(battlerAtk)] == ABILITY_OBLIVIOUS))
                    ADJUST_SCORE(-10);
            }
            break;
        case EFFECT_SANDSTORM:
        ...
        case EFFECT_MAGNITUDE:
            if (aiData->abilities[battlerDef] == ABILITY_LEVITATE || !IsBattlerGrounded(battlerDef))
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_MORNING_SUN:
            if ((AI_GetWeather() & (B_WEATHER_RAIN | B_WEATHER_HAIL | B_WEATHER_SNOW)))
                ADJUST_SCORE(-2);
        ...
        case EFFECT_PROTECT:
            {
                  ...
                  && move != MOVE_CRAFTY_SHIELD) //These moves have infinite usage
                {
                    ...
                    else if (aiData->abilities[battlerDef] == ABILITY_INFILTRATOR)
                    {
                        ADJUST_SCORE(-4);
                    }
                }
        ...
        case EFFECT_TAUNT:
            if (gDisableStructs[battlerDef].tauntTimer > 0
              || aiData->abilities[battlerDef] == ABILITY_OBLIVIOUS || aiData->abilities[battlerDef] == ABILITY_INNER_FOCUS
              || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_SCORE(-10);
            break;
        case EFFECT_BESTOW:
    ...
    if (moveTarget == MOVE_TARGET_FOES_AND_ALLY)
    {
        ...
        // Partner will not faint.
            ...
            case HOLD_EFFECT_WEAKNESS_POLICY:
                if (aiData->effectiveness[battlerAtk][battlerAtkPartner][gAiThinkingStruct->movesetIndex] >= UQ_4_12(1.6) && isFriendlyFireOK)
                {
                    ADJUST_SCORE(GOOD_EFFECT);
                }
                break;
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        ...
        if (!partnerProtecting && moveTarget != MOVE_TARGET_BOTH && !DoesBattlerIgnoreAbilityChecks(battlerAtk, aiData->abilities[battlerAtk], move))
        {
            ...
            case ABILITY_VOLT_ABSORB:
                ...
                {
                    if (B_REDIRECT_ABILITY_IMMUNITY < GEN_5 && (atkPartnerAbility == ABILITY_LIGHTNING_ROD || atkPartnerAbility == ABILITY_ORIGIN_OF_SEA))
                    {
                        RETURN_SCORE_MINUS(10);
                    }
            ...
            case ABILITY_MAGMA_ARMOR:
                if (moveType == TYPE_FIRE || moveType == TYPE_GRASS || moveType == TYPE_ICE)
                {
                    if (moveTarget == MOVE_TARGET_FOES_AND_ALLY)
                    {
                        ADJUST_SCORE(DECENT_EFFECT);
            ...
            case EFFECT_SKILL_SWAP:
                if (aiData->abilities[battlerAtk] != aiData->abilities[BATTLE_PARTNER(battlerAtk)] && !attackerHasBadAbility)
                {
                    ...
                    if (aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_TRUANT)
                    {
                        ...
                        || aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_UNNERVE
                        || aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_SILK_SPEW)
                    {
                        ADJUST_SCORE(DECENT_EFFECT);
                    }
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_EXPLOSION:
    case EFFECT_MEMENTO:
        if (gAiThinkingStruct->aiFlags[battlerAtk] & AI_FLAG_WILL_SUICIDE && gBattleMons[battlerDef].statStages[STAT_EVASION] < (DEFAULT_STAT_STAGE + 1))
        {
    ...
    case EFFECT_ATTACK_DOWN_2:
        ...
        if (gBattleMons[battlerDef].statStages[STAT_ATK] < DEFAULT_STAT_STAGE)
            ...
            ADJUST_SCORE(-1);
        if (gBattleMons[battlerDef].statStages[STAT_ATK] > (MIN_STAT_STAGE + 1) && !AI_RandLessThan(50))
            ADJUST_SCORE(-2);
    ...
    case EFFECT_DEFENSE_DOWN_2:
        if (!ShouldLowerDefense(battlerAtk, battlerDef, aiData->abilities[battlerDef]))
            ADJUST_SCORE(-2);
        if ((aiData->hpPercents[battlerAtk] < 70 && !AI_RandLessThan(50)) || (gBattleMons[battlerDef].statStages[STAT_DEF] <= (MIN_STAT_STAGE + 1) && !AI_RandLessThan(50)))
            ADJUST_SCORE(-2);
    ...
    case EFFECT_SPECIAL_ATTACK_DOWN_2:
        ...
        if (gBattleMons[battlerDef].statStages[STAT_SPATK] < DEFAULT_STAT_STAGE)
            ...
            ADJUST_SCORE(-1);
        if (gBattleMons[battlerDef].statStages[STAT_SPATK] > (MIN_STAT_STAGE + 1) && !AI_RandLessThan(50))
            ADJUST_SCORE(-2);
        else if (aiData->hpPercents[battlerDef] < 70)
            ADJUST_SCORE(-2);
        break;
    ...
    case EFFECT_SPECIAL_DEFENSE_DOWN_2:
        ...
        if ((aiData->hpPercents[battlerAtk] < 70 && !AI_RandLessThan(50))
          || (gBattleMons[battlerDef].statStages[STAT_SPDEF] <= (MIN_STAT_STAGE + 1) && !AI_RandLessThan(50)))
            ADJUST_SCORE(-2);
        if (aiData->hpPercents[battlerDef] <= 70)
            ADJUST_SCORE(-2);
        break;
    ...
    case EFFECT_ACCURACY_DOWN_2:
        ...
        if ((aiData->hpPercents[battlerAtk] < 70 || aiData->hpPercents[battlerDef] < 70) && AI_RandLessThan(100))
            ADJUST_SCORE(-1);
        if (gBattleMons[battlerDef].statStages[STAT_ACC] <= (MIN_STAT_STAGE + 1) && !AI_RandLessThan(80))
            ADJUST_SCORE(-2);
    ...
    case EFFECT_EVASION_DOWN_2:
        if (!ShouldLowerEvasion(battlerAtk, battlerDef, aiData->abilities[battlerDef]))
            ADJUST_SCORE(-2);
        if ((aiData->hpPercents[battlerAtk] < 70 || gBattleMons[battlerDef].statStages[STAT_EVASION] <= (MIN_STAT_STAGE + 1)) && !AI_RandLessThan(50))
            ADJUST_SCORE(-2);
        if (aiData->hpPercents[battlerDef] <= 70)
            ADJUST_SCORE(-2);
    ...
    case EFFECT_ROAR:
        if ((IsSoundMove(move) && aiData->abilities[battlerDef] == ABILITY_SOUNDPROOF)
          || (IsSoundMove(move) && IsBattleMoveStatus(move) && aiData->abilities[battlerDef] == ABILITY_OBLIVIOUS)
    ...
    case EFFECT_MORNING_SUN:
    case EFFECT_SYNTHESIS:
        if (ShouldRecover(battlerAtk, battlerDef, move, 37, AI_DEFENDING))
            ADJUST_SCORE(GOOD_EFFECT);
        break;
    case EFFECT_ROOST:
        ...
        if (AI_IsFaster(battlerAtk, battlerDef, move) && move == MOVE_ROOST && predictedMove != MOVE_NONE
            && (predictedType == TYPE_ELECTRIC || predictedType == TYPE_ROCK))
            ADJUST_SCORE(DECENT_EFFECT); // If using Roost would negate Flying weakness
        break;
    case EFFECT_LIGHT_SCREEN:
    case EFFECT_REFLECT:
    case EFFECT_AURORA_VEIL:
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100, AI_DEFENDING))
        {
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
              ...
              || aiData->abilities[battlerAtk] == ABILITY_INNER_FOCUS)
    ...
    case EFFECT_HAIL:
        if (ShouldSetHail(battlerAtk, aiData->abilities[battlerAtk], aiData->holdEffects[battlerAtk]))
        {
            ...
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_ICY_ROCK)
                ADJUST_SCORE(WEAK_EFFECT);
            if (HasMoveWithEffect(battlerDef, EFFECT_MORNING_SUN) //   || HasMoveWithEffect(battlerDef, EFFECT_MOONLIGHT))
              || HasMoveWithEffect(battlerDef, EFFECT_SYNTHESIS))
                ADJUST_SCORE(WEAK_EFFECT);
        }
        break;
    case EFFECT_SNOWSCAPE:
        if (ShouldSetSnow(battlerAtk, aiData->abilities[battlerAtk], aiData->holdEffects[battlerAtk]))
        {
            ...
            if (HasMoveWithEffect(battlerDef, EFFECT_MORNING_SUN) //   || HasMoveWithEffect(battlerDef, EFFECT_MOONLIGHT))
              || HasMoveWithEffect(battlerDef, EFFECT_SYNTHESIS))
                ADJUST_SCORE(WEAK_EFFECT);
            if (HasMoveWithType(battlerDef, TYPE_GRASS) || HasMoveWithType(BATTLE_PARTNER(battlerDef), TYPE_GRASS))
                ADJUST_SCORE(WEAK_EFFECT);
        }
        break;
    case EFFECT_RAIN_DANCE:
        if (ShouldSetRain(battlerAtk, aiData->abilities[battlerAtk], aiData->holdEffects[battlerAtk]))
        {
            ...
            if (HasMoveWithEffect(battlerDef, EFFECT_MORNING_SUN)
            //   || HasMoveWithEffect(battlerDef, EFFECT_SYNTHESIS)
              || HasMoveWithEffect(battlerDef, EFFECT_SHORE_UP)
            //   || HasMoveWithEffect(battlerDef, EFFECT_MOONLIGHT)
    ...
    case EFFECT_SUNNY_DAY:
        if (ShouldSetSun(battlerAtk, aiData->abilities[battlerAtk], aiData->holdEffects[battlerAtk]))
        {
            ...
            if (HasMoveWithType(battlerDef, TYPE_WATER) || HasMoveWithType(BATTLE_PARTNER(battlerDef), TYPE_WATER)
                || HasMoveWithType(battlerDef, TYPE_ICE) || HasMoveWithType(BATTLE_PARTNER(battlerDef), TYPE_ICE))
                ADJUST_SCORE(WEAK_EFFECT);
    ...
    case EFFECT_RECYCLE:
        ...
        if (IsRecycleEncouragedItem(GetUsedHeldItem(battlerAtk)))
            ADJUST_SCORE(WEAK_EFFECT);
        if (aiData->abilities[battlerAtk] == ABILITY_CHEEK_POUCH)
        {
    ...
    case EFFECT_WATER_SPORT:
        ...
        break;
    case EFFECT_TICKLE:
        if (gBattleMons[battlerDef].statStages[STAT_DEF] > (MIN_STAT_STAGE + 1) && HasMoveWithCategory(battlerAtk, DAMAGE_CATEGORY_PHYSICAL)
    ...
    case EFFECT_ION_DELUGE:
        if ((aiData->abilities[battlerAtk] == ABILITY_VOLT_ABSORB
          ...
          || (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5 && aiData->abilities[battlerAtk] == ABILITY_ORIGIN_OF_SEA))
    ...
    case EFFECT_MIRROR_COAT:
        ...
        break;
    case EFFECT_SHORE_UP:
        if ((AI_GetWeather() & B_WEATHER_SANDSTORM) && ShouldRecover(battlerAtk, battlerDef, move, 50, AI_DEFENDING))
            ADJUST_SCORE(DECENT_EFFECT);
        else if (ShouldRecover(battlerAtk, battlerDef, move, 37, AI_DEFENDING))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_ENDEAVOR:
...
static s32 AI_HPAware(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        if ((effect == EFFECT_HEAL_PULSE || effect == EFFECT_HIT_ENEMY_HEAL_ALLY)
         ...
         || (moveType == TYPE_FIRE && gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_FLASH_FIRE)
         || (moveType == TYPE_GROUND && gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_EARTH_EATER)
         || (moveType == TYPE_GRASS && gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_SAP_SIPPER)
         || (moveType == TYPE_ICE && gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_ICE_ABSORB)
         || (moveType == TYPE_POISON && gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_GUNK_MUNCHER)
         || (moveType == TYPE_WATER && (gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_DRY_SKIN || gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_WATER_ABSORB
            || gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_HYDRATION || gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_STORM_DRAIN)))
        {
            if (gStatuses3[battlerDef] & STATUS3_HEAL_BLOCK)
                return 0;
