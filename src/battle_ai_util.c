...
static inline void CalcDynamicMoveDamage(struct DamageCalculationData *damageCalcData, u16 *medianDamage, u16 *minimumDamage, u16 *maximumDamage, enum ItemHoldEffect holdEffectAtk, u32 abilityAtk, u32 abilityDef)
{
...
bool32 ShouldLowerAttack(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
    ...
    if (gBattleMons[battlerDef].statStages[STAT_ATK] > (DEFAULT_STAT_STAGE - 2)
...
bool32 ShouldLowerDefense(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
    ...
    if (gBattleMons[battlerDef].statStages[STAT_DEF] > (DEFAULT_STAT_STAGE - 2)
...
bool32 ShouldLowerSpAtk(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
    ...
    if (gBattleMons[battlerDef].statStages[STAT_SPATK] > (DEFAULT_STAT_STAGE - 2)
...
static bool32 PartyBattlerShouldAvoidHazards(u32 currBattler, u32 switchBattler)
{
    ...
    bool32 isAirborne = gSpeciesInfo[species].airborne;
    u32 maxHp = GetMonData(mon, MON_DATA_MAX_HP);

    if (flags == 0)
        return FALSE;

    if (ability == ABILITY_MAGIC_GUARD || ability == ABILITY_BATTLE_ARMOR || ability == ABILITY_SHELL_ARMOR || ability == ABILITY_SOLID_ROCK)
        return FALSE;
    ...
    if (flags & SIDE_STATUS_SPIKES && ((type1 != TYPE_FLYING && type2 != TYPE_FLYING
        && ability != ABILITY_LEVITATE && !isAirborne && holdEffect != HOLD_EFFECT_AIR_BALLOON)
...
{
    bool32 hasStatBoost = AnyUsefulStatIsRaised(battlerAtk) || gBattleMons[battlerDef].statStages[STAT_EVASION] >= (MAX_STAT_STAGE - 1); //Significant boost in evasion for any class
    u32 battlerToSwitch;
...
u32 ShouldTryToFlinch(u32 battlerAtk, u32 battlerDef, u32 atkAbility, u32 defAbility, u32 move)
{
    ...
    else if ((atkAbility == ABILITY_SUPER_LUCK
      || atkAbility == ABILITY_CRAFTY
...
static enum AIScore IncreaseStatUpScoreInternal(u32 battlerAtk, u32 battlerDef, enum StatChange statId, bool32 considerContrary)
{
    ...
    case STAT_CHANGE_ACC:
        if (gBattleMons[battlerAtk].statStages[STAT_ACC] <= (DEFAULT_STAT_STAGE - 2)) // Increase only if necessary
            tempScore += DECENT_EFFECT;
        break;
...
void IncreasePoisonScore(u32 battlerAtk, u32 battlerDef, u32 move, s32 *score)
{
    ...
    if (AI_CanPoison(battlerAtk, battlerDef, gAiLogicData->abilities[battlerDef], move, gAiLogicData->partnerMove) && gAiLogicData->hpPercents[battlerDef] > 20)
    {
        ...
        if (IsPowerBasedOnStatus(battlerAtk, EFFECT_DOUBLE_POWER_ON_ARG_STATUS, STATUS1_PSN_ANY)
         ...
         || gAiLogicData->abilities[battlerAtk] == ABILITY_MERCILESS || gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_MERCILESS)
            ADJUST_SCORE_PTR(DECENT_EFFECT);
        else
            ADJUST_SCORE_PTR(WEAK_EFFECT);
    }
